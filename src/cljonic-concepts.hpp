...
concept NothrowCopyableElement =
    CopyableElement<T> && requires(T value, const T &other) {
      { T{} } noexcept;
      { T{other} } noexcept;
...
concept NothrowElementConstruction =
    std::convertible_to<Arg, T> && requires(Arg argument) {
...
concept TotallyOrdered =
    StableEqualityComparable<T> && requires(const T &a, const T &b) {
      { a < b } -> std::same_as<bool>;
    };
...
concept CljonicVector =
    CljonicCollection<T> && (concepts_detail::collection_kind_of_v<T> ==
...
concept IndexedCollection =
    SequenceableCollection<C> && requires(const C &c, std::size_t i) {
      { c(i) } noexcept;
