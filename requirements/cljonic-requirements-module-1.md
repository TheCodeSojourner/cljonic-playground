...
REQ-VAL-007A. Every user-defined type stored as a collection element, map key, or map value MUST satisfy the `NothrowCollectionElement` storage contract: default construction, copy construction, copy assignment, and destruction MUST be non-throwing. This storage-admission contract MUST remain independent of equality, ordering, hashing, parsing, traversal, and other operation-specific capabilities.

REQ-VAL-008. Every public cljonic semantic operation that returns an owning cljonic result MUST return a self-contained owning value whose validity does not depend on the lifetime or state of another value, temporary, external storage, or hidden borrowed state. Public cljonic semantic operations MUST NOT return `std::ranges` views, `std::span` values, `std::string_view` values, or other borrowed lazy views. Explicit collection-owned C++ interoperability accessors are the sole view exception: they MUST be const-qualified observers, MUST expose only read-only elements or characters, MUST NOT provide mutable references or iterators, and MUST satisfy the bounded, non-mutating lifetime and invalidation requirements defined for interoperability. Explicit producer values are a separate permitted result category governed by the producer requirements; they MUST own their parameters, MUST NOT own materialized result storage, and MUST NOT borrow source storage or hidden mutable state.
