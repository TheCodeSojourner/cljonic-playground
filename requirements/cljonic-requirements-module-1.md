...
REQ-VAL-008. Every public cljonic semantic operation that returns a cljonic result MUST return a self-contained owning value whose validity does not depend on the lifetime or state of another value, temporary, external storage, or hidden borrowed state. Public cljonic semantic operations MUST NOT return `std::ranges` views, `std::span` values, `std::string_view` values, or other borrowed lazy views. Explicit collection-owned C++ interoperability accessors are the sole exception: they MUST be const-qualified observers, MUST expose only read-only elements or characters, MUST NOT provide mutable references or iterators, and MUST satisfy the bounded, non-mutating lifetime and invalidation requirements defined for interoperability.
