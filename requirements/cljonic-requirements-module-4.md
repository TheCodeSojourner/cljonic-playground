...
REQ-FN-027A. A supported collection MAY provide direct construction from a compatible bounded C++ source when its destination type and capacity are encoded by the constructed collection type. Direct source construction MUST copy source elements or converted characters into owned collection storage, MUST leave the source unchanged, and MUST remain non-allocating, non-throwing, and independent of the source lifetime. When the source extent is a compile-time constant greater than the destination capacity, construction MUST fail at compile time. When the source extent is known only at runtime and exceeds capacity, direct construction MUST return the deterministic bounded prefix permitted by the destination capacity. Direct source construction MUST be documented as bounded construction rather than complete materialization and MUST NOT introduce an optional, result, exception, or other checked-constructor error channel.
