...
REQ-PLAT-017. Collection types MUST provide a non-mutating, non-allocating, non-throwing, `constexpr`-capable traversal mechanism sufficient for cljonic free-function implementations to visit each active logical element without depending on collection-specific storage details. The traversal mechanism MUST expose only active elements and MUST preserve each collection's documented logical traversal order.

REQ-PLAT-018. Collection types MUST provide explicitly identified, const-qualified, read-only C++ interoperability accessors where a standard representation is useful, including `std::span<const T>`-like access for contiguous logical elements and `std::string_view`-like access for string content where applicable. These accessors MUST be non-allocating, non-throwing, `constexpr`-capable where supported, MUST expose only active logical content, and MUST NOT provide mutable references, mutable iterators, or mutable views into collection storage.
...
REQ-PLAT-022. C++ interoperability accessors for `Vector`, `Map`, `Set`, `Queue`, and `String` MUST expose the active logical range in the collection's documented representation: vector, set, and queue elements; map entries; and string content excluding its null terminator. A queue accessor MUST preserve logical FIFO order, and map/set accessors MUST NOT imply an ordered semantic contract where none is specified.
