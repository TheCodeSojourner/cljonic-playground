...
REQ-PLAT-018. Collection types MAY provide explicitly identified, const-qualified, read-only C++ interoperability accessors where a standard representation is useful. A collection MAY provide `std::span<const T>`-like access only when its active logical elements are represented as one contiguous range; a `String` MAY provide `std::string_view`-like access for its stored content excluding its null terminator. These accessors MUST be non-allocating, non-throwing, `constexpr`-capable where supported, MUST expose only active logical content, and MUST NOT provide mutable references, mutable iterators, or mutable views into collection storage.
