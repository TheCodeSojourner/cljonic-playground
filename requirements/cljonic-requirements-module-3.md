...
REQ-COLL-002A. A `Vector<T, N>` MUST be callable with `operator()(Index, fallback)` when `Index` and `T` satisfy the same capabilities required by bounded indexed lookup; `fallback` MUST default to `T{}` when omitted. The operation MUST return the element at a valid index or the fallback value when the index is invalid. It MUST NOT mutate the vector, allocate, throw, or change vector order or traversal state. `contains(vector, index)` MUST remain the authoritative way to distinguish an invalid index from a valid index whose element equals `T{}`; `get(vector, index)` and `get(vector, index, fallback)` MUST remain behaviorally equivalent free-function forms. Negative indexes, when representable by the accepted index type, MUST be invalid.
...
REQ-COLL-004B. A `Map<K, V, N>` MUST be callable with `operator()(K, fallback)` when `K` and `V` satisfy the same capabilities required by map lookup; `fallback` MUST default to `V{}` when omitted. The operation MUST return the associated value for a present key or the fallback value when the key is absent. It MUST NOT mutate the map, insert a key, allocate, throw, or change traversal state. `contains(map, key)` MUST remain the authoritative way to distinguish a missing key from a present key whose value equals `V{}`; `get(map, key)` and `get(map, key, fallback)` MUST remain behaviorally equivalent free-function forms.
...
REQ-COLL-005B. A `Set<T, N>` MUST be callable with `operator()(T, fallback)` when `T` satisfies the stable equality capability required by set membership; `fallback` MUST default to `T{}` when omitted. The operation MUST return the matching stored element for a present value or the fallback value when the value is absent. It MUST NOT mutate the set, insert a value, allocate, throw, reorder elements, or change traversal state. `contains(set, value)` MUST remain the authoritative boolean membership predicate and MUST distinguish an absent value from a present value equal to `T{}`; `get(set, value)` and `get(set, value, fallback)` MUST remain behaviorally equivalent free-function forms.

REQ-COLL-012A. A `String<N>` MUST be callable with `operator()(Index, fallback)` for indexed lookup, where `Index` is any integral type and `fallback` defaults to `char{}` when omitted. The operation MUST return the stored ASCII byte at a valid content index or the fallback character when the index is invalid. Negative and unrepresentable indexes MUST be invalid rather than converted modulo to the unsigned key domain. The operation MUST NOT expose the null terminator as a content element, mutate the string, allocate, or throw. `contains(string, index)` MUST remain the authoritative way to distinguish an invalid index from a valid index whose byte equals `char{}`.
...
REQ-COLL-020O. For `String<N>`, an `assoc` index of any integral type MUST be valid only when it is representable by the normalized `std::size_t` key domain, non-negative, no greater than the source string's logical content count, and either less than the logical content count or equal to the logical content count when content capacity remains. An invalid index, including a negative or unrepresentable index, an index beyond the append position, or an append index at full content capacity, MUST produce an unchanged string without throwing, allocating, or mutating the source. The null terminator MUST NOT be a valid association or lookup index.

REQ-COLL-020P. For `String<N>`, `can_assoc(string, index)` MUST accept any integral index type, normalize only indexes representable by the `std::size_t` key domain, and return true for an existing content index or the logical content-count append index when content capacity remains. It MUST return false for negative, unrepresentable, invalid, or full-capacity append indexes. Its result MUST be independent of the character value that would be associated.
...
REQ-FN-002S. The callable `String<N>` lookup operation specified by `REQ-COLL-012A` MUST be equivalent to `get(string, index)` and `get(string, index, fallback)`, with the fallback defaulting to `char{}` when omitted. `contains(string, index)` MUST be non-throwing, non-allocating, and consistent with the callable lookup operation; the null terminator MUST remain outside the lookup domain.
