...
TEST_CASE("Range supports the bounded integer producer subset", "[range]")
{
    ...
    STATIC_REQUIRE(!requires { by_default.start(); });
    STATIC_REQUIRE(!requires { by_default.end(); });
    STATIC_REQUIRE(!requires { by_default.step(); });
    STATIC_REQUIRE(!requires { Range{0.0F, 1.0F}; });
    STATIC_REQUIRE(!requires { Range{0.0, 10.0, 1.0}; });
    STATIC_REQUIRE(!requires { Range{0, 10, 1.0F}; });
    STATIC_REQUIRE(!requires { Range{0, 10, std::numeric_limits<float>::epsilon()}; });
    ...
    CHECK(count(zero_step_equal_end) == CLJONIC_COLLECTION_MAXIMUM_ELEMENT_COUNT);
