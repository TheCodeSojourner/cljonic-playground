...
TEST_CASE("MapEntry holds value-semantic key/value pair", "[map-entry]") {
  ...
  STATIC_REQUIRE(entry.valid_index(0));
  STATIC_REQUIRE(entry.valid_index(1));
  STATIC_REQUIRE_FALSE(entry.valid_index(2));

  const auto runtime_entry = MapEntry<int, int>{2, 200};
  CHECK(runtime_entry.key == 2);
  CHECK(runtime_entry.value == 200);
  CHECK(runtime_entry.valid_index(0));
  CHECK(runtime_entry.valid_index(1));
  CHECK_FALSE(runtime_entry.valid_index(2));
  ...
  CHECK_FALSE(runtime_entry.valid_index(negative_index));
  volatile int too_large_index = 2;
  CHECK_FALSE(runtime_entry.valid_index(too_large_index));
}
