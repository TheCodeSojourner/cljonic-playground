...
- When should the single-header equivalence and no-heap probes be added to the automated test targets?
 ...
 - Should the generated `cljonic.hpp` be committed as a release artifact or regenerated by release automation?
...
2. Decide whether release packaging should commit or regenerate `cljonic.hpp`.
3. Preserve the current green baseline with focused validation followed by `make all` after substantive changes.



 1. Decide release ownership and commit/regeneration policy for root-level `cljonic.hpp`.
