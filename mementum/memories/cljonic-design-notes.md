...
- Producer equality is producer parameter equality (REQ-FN-014B): `operator==` + `parameters_equal` on Range/Repeat/Cycle compare stored parameters only (never the produced sequence); Iterate/Repeatedly excluded (callable step). Distinct parameters compare unequal even when sequences coincide. See `producer-parameter-equality.md`.
- The `contains_floating_point` / `contains_callable` walkers recurse through std::variant, collections, MapEntry, and producer stored parameters; callable components (including function pointers) are always rejected for stable equality.
