...
- Level 2 `CapabilityConcept` (semantic gates, layered on nominal identity): `SequenceableCollection` (`count`/`is_empty`), `IndexedCollection` (`operator(i)`/`contains`), `AssociativeCollection` (`operator(k)`/`contains`), plus value concepts `StableEqualityComparable` and `TotallyOrdered`.
