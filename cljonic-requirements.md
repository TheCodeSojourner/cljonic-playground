...
REQ-FN-009. `cycle`, `iterate`, `range`, `repeat`, and `repeatedly` MUST be standalone producer values or producer operations that do not own materialized result storage or require a result-capacity template parameter. Their values MUST be materialized through `into` into an explicit bounded destination. Semantically infinite producers MUST carry a bounded synthesis cap equal to `CLJONIC_COLLECTION_MAXIMUM_ELEMENT_COUNT`; the cap is a runtime safety ceiling for bounded materialization and truncation, not a claim that the producer is finite. Finite forms MUST produce their complete result when it fits the destination.

REQ-FN-010. For `cycle`, `iterate`, `range` without a finite end, `repeat` without a count, and `repeatedly` without a count, complete-result preflight MUST report that the result does not fit unless the operation terminates before reaching its unbounded behavior. Counted `repeat` and `repeatedly`, and finite `range`, MUST use their runtime result count for preflight and bounded `into` materialization. Any semantically infinite producer MUST be bounded by `CLJONIC_COLLECTION_MAXIMUM_ELEMENT_COUNT` during synthesis and MUST NEVER materialize more than that cap. A finite `range` whose requested result count exceeds the cap, when constructed outside a required constant expression, MUST silently degrade to a deterministic bounded prefix of at most `CLJONIC_COLLECTION_MAXIMUM_ELEMENT_COUNT` elements; its effective exclusive end MUST be adjusted to the boundary after that bounded prefix, while `count` reports the bounded prefix size. The effective size MUST be the authoritative bound for future iteration and materialization, and all canonical observation of that bound MUST use the applicable free functions. See REQ-FN-012E for the case where such a `range` is constructed as part of a required constant expression.

REQ-FN-011. `Range`, `Repeat`, `Cycle`, `Iterate`, and `Repeatedly` producer values MUST own their parameters and MUST remain valid independently of other values. They MUST NOT allocate result storage or retain borrowed dependencies on source collections, callbacks, or input values. A producer's `count` value, when exposed, MUST be interpreted as a synthesis cap or safety ceiling, not as a semantic cardinality.
...
REQ-FN-012E. A finite `range` whose requested result count exceeds `CLJONIC_COLLECTION_MAXIMUM_ELEMENT_COUNT` MUST be rejected at compile time when its construction is evaluated as part of a required constant expression (for example, a `constexpr` object, `static_assert`, or template argument), rather than silently degrading to a bounded prefix as REQ-FN-010 requires at runtime. This compile-time rejection applies only to finite oversized forms; the zero-step and other semantically infinite forms remain governed by REQ-FN-012 and REQ-FN-010.
...
REQ-PLAT-011. Compile-time regex support MUST use CTRE as the canonical audited implementation for cljonic. Equivalent implementations MAY be used only when they are explicitly approved and preserve the same no-allocation, no-exception, bounded-storage, diagnostic, and licensing requirements.

REQ-PLAT-011A. The cljonic project MUST select CTRE as the default compile-time regex library for the supported regular-expression surface. A runtime regex profile MAY be offered as a separate optional capability, but the default supported regex profile MUST remain CTRE-based unless a separate approved requirement changes the selected library.
...
REQ-VOCAB-010A. The cljonic keyword model MUST be global-only and MUST NOT require namespaced keyword identifiers. Keyword values MUST be drawn from the project-defined bounded keyword catalog and MUST be represented using a stable enumeration or equivalent fixed catalog model rather than ad hoc runtime symbol resolution.

REQ-VOCAB-010B. When a map uses a keyword key, the key MUST come from the project-defined keyword catalog and MUST be represented by the same stable enumeration or equivalent fixed catalog model used by the keyword system. Non-keyword map keys MAY be supported only when the applicable collection capability requirements explicitly permit them; no implicit runtime keyword synthesis or namespaced-key conversion is allowed.
